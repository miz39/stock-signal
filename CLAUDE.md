...
Claude Code がこのプロジェクトで起動したとき、**まず最初に以下を確認して実行すること**:

```python
from datetime import date
dow = date.today().weekday()  # 5=土, 6=日
```

**土曜（5）または日曜（6）の場合 → 週次レビュー＋改善サイクルを自動実行する:**

### ステップ1: 週次レビュー
1. `python3 generate_trade_journal.py` — 今週のジャーナル生成
2. 今週（月〜金）のクローズ・エントリー・オープンポジションを `trades.json` から集計
3. 週間PnL・勝率・PF・注目ポイントをユーザーに報告
4. `CHANGELOG.md` の「週次レビュー記録」テーブルに今週の行を追記
5. `bash push.sh "Weekly review: YYYY-WXX"` で push

### ステップ2: パターン分析（自動）
`trades.json` の直近20件のクローズトレードを分析し、以下を集計する:
- **セクター別** 勝率・PF（PF < 1.0 のセクターを要注意としてマーク）
- **保有日数別** 損益分布（短期 <7日 / 中期 7-20日 / 長期 >20日）
- **exit_reason別** 集計（損切り率・平均損失額）
- **エントリー時 RSI/ADX 分布**（負けトレードに偏りがないか）

### ステップ3: 改善候補の導出（Claude が判断）
パターン分析の結果から、以下のパラメータに問題があれば改善候補として提示する:
- `rsi_entry_max` / `rsi_entry_min`（RSI帯域の偏り）
- `adx_threshold`（ADX閾値とトレンド強度の関係）
- `stop_loss_pct` / `trail_activation_pct`（損切り・トレーリングの距離）
- `time_stop_days`（タイムストップの日数）

**改善なしと判断した場合はその旨を報告して終了する。**

### ステップ4: バックテスト検証
改善候補がある場合、`python3 backtest_improved.py --mode sensitivity` を実行し、
現行パラメータとの比較（PF・勝率・最大DD）をユーザーに提示する。

### ステップ5: ユーザー承認 → 実装
- **パラメータ変更（`config.yaml`）は必ずユーザー承認を取ってから実行する**
- **ロジック変更（`.py` ファイル）は明示的に依頼された場合のみ実行する**
- 承認後: `config.yaml` を更新 → `bash push.sh "Optimize params: YYYY-WXX"` で push

**平日の場合 → 通常通り対応する**（自動実行なし）。
