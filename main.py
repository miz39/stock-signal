...
def run(profile_name: str = "default"):
    ...
    # Slack通知: BUYシグナルのみ（リアル取引判断用、defaultプロファイルのみ）
    # SELL・日次サマリはDiscordのみ。Slackはリアル取引チャンネルとして使う。
    if slack_webhook_url and profile_name == "default" and buy_signals:
        buy_embeds = [e for e in embeds if any(
            k in e.get("title", "") for k in ["スキャン結果", "買い候補", "クラッシュ", "CRITICAL", "警告"]
        )]
        if buy_embeds:
            slack_text = format_signal_mrkdwn(content, buy_embeds)
